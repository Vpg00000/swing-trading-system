...
// T-322: Floating popup toasts disabled per user request - all logs routed to Main Pipeline Console
function showStickyLongTaskToast(id, name, startTime) {
    // Disabled: User requested no floating popup logs. Everything is displayed inside the main execution console.
    return;
}
...
function renderVisibleOpportunities() {
    ...
    visibleOpportunities.forEach((item, index) => {
        ...
        const action = item.action || item.suggested_action || (score >= 75 ? 'BUY_NOW' : (score >= 65 ? 'BUY' : 'WATCH'));
        ...
        const stopLoss = item.stop_loss || item.stop_price || (price ? (price * 0.95).toFixed(2) : '--');
        const targetPrice = item.target_price || (price ? (price * 1.15).toFixed(2) : '--');
        const rrRaw = item.rr_ratio;
        const rrRatio = (rrRaw && rrRaw !== '0.0' && rrRaw !== 0) 
            ? (String(rrRaw).startsWith('1:') ? String(rrRaw) : `1:${Number(rrRaw).toFixed(1)}`) 
            : '1:2.2';
